...
**Open questions for next session:** none blocking. Watch for the same drift happening in reverse — periodically diff this repo's `main` against Watchstander's the same way this session did, rather than only pushing parity checks one direction. Worth deciding: does the eval-harness item (tiered 5 by the strict security→stability→UX ordering, but independently flagged by three reviewers as highest-leverage) get pulled forward ahead of tier order, or does the tier order win as written? Not resolved this session — flagged honestly in ARCHITECTURE.md §9 rather than silently prioritized one way.
